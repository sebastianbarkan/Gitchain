...
export const createTask = createAsyncThunk(
	'task/createTask',
	async ({ taskLevel, signerLevel, bountyAmount, description, languages, category }, { getState, dispatch }) => {
		const taskId = uuidv4();
		...
		try {
			...
			while (attempts < maxAttempts) {
				attempts++;
				try {
					transactionReceipt = await tronWeb.trx.getConfirmedTransaction(transactionInfo.txID);
			...
			console.log("RECIPT", transactionReceipt)
